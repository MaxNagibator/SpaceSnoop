...
public class SyncFreshnessTests
{
    ...
    [Test]
    public void Голосование_по_изменённым_определяет_новее_сторону()
    {
        var leftWins = Modified("a.txt", New, Old);
        var alsoLeft = Modified("b.txt", New, Old);
        var rightWins = Modified("c.txt", Old, New);
        var root = Dir("", leftWins, alsoLeft, rightWins);
    ...
    [Test]
    public void Разница_в_пределах_FAT_допуска_не_голосует()
    {
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(summary.Verdict, Is.EqualTo(NewerSide.None));
        }
    }

    [Test]
    public void Равное_число_голосов_даёт_ничью()
    {
