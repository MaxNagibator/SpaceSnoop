...
public sealed class SpaceSnoopTools
{
    ...
    [Description("Как приложению живётся прямо сейчас: отклик интерфейса (задержка и пик за окно наблюдения), занятая память, число сборок мусора, скорость текущей операции и время последнего запуска приложения. По запросу добавляет историю наблюдения – ряд замеров за последние минуты, по которому видно, когда именно была просадка. Отвечает мгновенно, ничего не считает и не меняет – с этого стоит начинать разговор «почему тормозит».")]
